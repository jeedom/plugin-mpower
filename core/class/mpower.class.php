...
class mpower extends eqLogic {
    ...
    public function getmpowerInfo($sessid) {
        try {
            ...
            $mpowerinfo = exec($cmd);
            log::add('mpower','debug',$cmd);
            log::add('mpower','debug',$mpowerinfo);
            ...
            foreach ($jsonmpowerinfo['sensors'] as $sensor) {
                if ($this->getConfiguration('model') == "6power") {
                    ...
                    $energy = round($sensor['energy'],2);
                    $powerfactor = round($sensor['powerfactor'],2);
                } else {
				    $port = $sensor['port'];
                    $output = $sensor['state']['output'];
                    $power = round($sensor['state']['power'],2);
                    $voltage = round($sensor['state']['voltage'],2);
                    $current = round($sensor['state']['current'],2);
                    $energy = round($sensor['state']['energy'],2);
                    $powerfactor = round($sensor['state']['powerfactor'],2);
				}
                $statecmd = mpowerCmd::byEqLogicIdAndLogicalId($this->getId(),'etat'.$port);
                ...
                $powercmd = mpowerCmd::byEqLogicIdAndLogicalId($this->getId(),'power'.$port);
                ...
                $voltagecmd = mpowerCmd::byEqLogicIdAndLogicalId($this->getId(),'voltage'.$port);
                ...
                $currentcmd = mpowerCmd::byEqLogicIdAndLogicalId($this->getId(),'current'.$port);
                ...
                $powerfactorcmd = mpowerCmd::byEqLogicIdAndLogicalId($this->getId(),'powerfactor'.$port);
                if (is_object($powerfactorcmd)) {
                    if ($powerfactorcmd->execCmd() == null || $powerfactorcmd->execCmd() != $powerfactor) {
						$changed = true;
                        $powerfactorcmd->event($powerfactor);
                    }
                }
	...
	public function toHtml($_version = 'dashboard') {
		...
		$html = template_replace($replace, getTemplate('core', $version, strtolower($this->getConfiguration('model')), 'mpower'));
